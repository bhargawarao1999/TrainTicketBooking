...
public class Ticket {
	...
	private double calcPassengerFare(Passenger passenger,int trainNumber) throws Exception
	{
		...
		else if(passenger.getPassengerAge()<12)
		{
			fare=trainfare*0.5;
		}
		else if(passenger.getPassengerGender()=='F')
		{
			fare = trainfare*0.75;
		}
		else 
	...
	private StringBuilder generateTicket(int trainNumber) throws Exception
	{
		...
		while(it.hasNext()) {
			...
			ticketPrinter[i]=passenger.getKey().getPassengerName()+"\t\t"+passenger.getKey().getPassengerAge()+"\t\t"+passenger.getKey().getPassengerGender()+"\t\t"+passenger.getValue()+"\n";
			i++;    
		}
